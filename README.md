<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Fida Fathima Portfolio</title>
</head>
<body>

    <!-- Header Section -->
    <header>
        <h1>Fida Fathima</h1>
        <h2>Student Portfolio</h2>
        <p>
            Welcome to my personal portfolio website. My name is Fida Fathima, and I am
            currently pursuing a degree in Electronics and Instrumentation Engineering at
            Federal Institute of Science and Technology (FISAT). This portfolio highlights
            my educational background, interests, skills, projects, achievements, and
            contact information.
        </p>

        <nav>
            <a href="#about">About Me</a> |
            <a href="#education">Education</a> |
            <a href="#skills">Skills</a> |
            <a href="#projects">Projects</a> |
            <a href="#gallery">Gallery</a> |
            <a href="#media">Media</a> |
            <a href="#contact">Contact</a>
        </nav>
        <hr>
    </header>

    <!-- About Section -->
    <section id="about">
        <h2>About Me</h2>

        <p>
            I am Fida Fathima, an enthusiastic and dedicated Electronics and Instrumentation
            Engineering student at Federal Institute of Science and Technology. I am passionate
            about technology, automation, electronics, industrial instrumentation, and modern
            engineering solutions. My academic journey has helped me develop analytical thinking,
            problem-solving abilities, and a strong foundation in engineering principles.
        </p>

        <p>
            Apart from academics, I enjoy learning about emerging technologies, participating
            in technical workshops, and working on innovative projects. I believe that continuous
            learning and practical experience are essential for becoming a successful engineer.
            My goal is to contribute to technological advancements that improve efficiency,
            sustainability, and quality of life.
        </p>
    </section>

    <hr>

    <!-- Education Section -->
    <section id="education">
        <h2>Educational Background</h2>

        <p>
            Currently, I am pursuing my Bachelor's degree in Electronics and Instrumentation
            Engineering from Federal Institute of Science and Technology (FISAT). My coursework
            includes electronics, sensors and transducers, control systems, industrial automation,
            microprocessors, embedded systems, and signal processing.
        </p>

        <table border="1">
            <caption><strong>Education Details</strong></caption>
            <tr>
                <th>Qualification</th>
                <th>Institution</th>
                <th>Status</th>
            </tr>
            <tr>
                <td>B.Tech Electronics and Instrumentation</td>
                <td>Federal Institute of Science and Technology (FISAT)</td>
                <td>Currently Studying</td>
            </tr>
            <tr>
                <td>Higher Secondary Education</td>
                <td>Completed</td>
                <td>Passed</td>
            </tr>
            <tr>
                <td>Secondary Education</td>
                <td>Completed</td>
                <td>Passed</td>
            </tr>
        </table>
    </section>

    <hr>

    <!-- Skills Section -->
    <section id="skills">
        <h2>Technical Skills</h2>

        <ul>
            <li>Electronics Fundamentals</li>
            <li>Instrumentation Systems</li>
            <li>Industrial Automation</li>
            <li>Control Systems</li>
            <li>Embedded Systems</li>
            <li>Programming Basics in C</li>
            <li>HTML Fundamentals</li>
            <li>Problem Solving</li>
        </ul>

        <h3>Personal Skills</h3>

        <ol>
            <li>Communication Skills</li>
            <li>Teamwork</li>
            <li>Leadership</li>
            <li>Time Management</li>
            <li>Critical Thinking</li>
        </ol>
    </section>

    <hr>

    <!-- Projects Section -->
    <section id="projects">
        <h2>Projects</h2>

        <article>
            <h3>Smart Monitoring System</h3>
            <p>
                This project focuses on monitoring environmental parameters such as temperature,
                humidity, and pressure using sensors and microcontrollers. The collected data
                can be displayed and analyzed for industrial applications.
            </p>
        </article>

        <article>
            <h3>Industrial Automation Study</h3>
            <p>
                This project explores the importance of automation in industries. It examines
                the role of programmable logic controllers (PLCs), sensors, actuators, and
                control systems in improving efficiency and reducing manual effort.
            </p>
        </article>
    </section>

    <hr>

    <!-- Image Section -->
    <section id="gallery">
        <h2>Gallery</h2>

        <figure>
            <img src="profile.jpg" alt="Fida Fathima Profile Photo" width="250">
            <figcaption>My Profile Picture</figcaption>
        </figure>

        <p>
            The image above represents my profile. Images are useful for visually presenting
            information about a person, project, or achievement.
        </p>
    </section>

    <hr>

    <!-- Audio and Video Section -->
    <section id="media">
        <h2>Audio and Video</h2>

        <h3>Audio Introduction</h3>
        <audio controls>
            <source src="introduction.mp3" type="audio/mpeg">
            Your browser does not support audio.
        </audio>

        <p>
            This audio file can contain a brief self-introduction describing my educational
            background, interests, and career aspirations.
        </p>

        <h3>Project Demonstration Video</h3>
        <video width="500" controls>
            <source src="project-demo.mp4" type="video/mp4">
            Your browser does not support video.
        </video>

        <p>
            The video can showcase demonstrations of engineering projects, technical
            presentations, or workshop activities.
        </p>
    </section>

    <hr>

    <!-- Links Section -->
    <section>
        <h2>Useful Links</h2>

        <p>
            Visit the official website of
            <a href="https://fisat.ac.in" target="_blank">
                Federal Institute of Science and Technology
            </a>.
        </p>

        <p>
            Connect with me through:
        </p>

        <ul>
            <li><a href="https://www.linkedin.com" target="_blank">LinkedIn</a></li>
            <li><a href="https://github.com" target="_blank">GitHub</a></li>
            <li><a href="mailto:fidafathima@example.com">Email Me</a></li>
        </ul>
    </section>

    <hr>

    <!-- Contact Form -->
    <section id="contact">
        <h2>Contact Me</h2>

        <p>
            If you would like to get in touch with me regarding academic collaborations,
            projects, internships, or professional opportunities, please fill out the form below.
        </p>

        <form action="#" method="post">

            <label for="name">Full Name:</label><br>
            <input type="text" id="name" name="name"><br><br>

            <label for="email">Email Address:</label><br>
            <input type="email" id="email" name="email"><br><br>

            <label for="phone">Phone Number:</label><br>
            <input type="tel" id="phone" name="phone"><br><br>

            <label for="gender">Gender:</label><br>

            <input type="radio" id="male" name="gender">
            <label for="male">Male</label>

            <input type="radio" id="female" name="gender">
            <label for="female">Female</label><br><br>

            <label for="course">Course:</label><br>
            <select id="course" name="course">
                <option>Electronics and Instrumentation</option>
                <option>Electronics and Communication</option>
                <option>Computer Science</option>
            </select><br><br>

            <label for="message">Message:</label><br>
            <textarea id="message" rows="5" cols="40"></textarea><br><br>

            <input type="checkbox" id="agree">
            <label for="agree">I agree to submit my details</label><br><br>

            <input type="submit" value="Submit">
            <input type="reset" value="Reset">

        </form>
    </section>

    <hr>

    <!-- Footer -->
    <footer>
        <p>
            © 2026 Fida Fathima. All Rights Reserved.
        </p>

        <p>
            Electronics and Instrumentation Engineering Student |
            Federal Institute of Science and Technology (FISAT)
        </p>
    </footer>

</body>
</html>
     

       

       
